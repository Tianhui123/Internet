...
namespace TianHui
{
	...
	class mLambda<Re(Args...)>final :TianHui::noCopy
	{
	public:
		template<typename L>
	...
	class ThreadPool final :TianHui::noCopy
	{
	...
	public:
		...
		void start(int size = 4)
		{
			...
			{
				threadPool_.push(std::make_shared<Thread>(mLambda<void()>([=]() {this->taskFunction(); })));
			}
	...
	private:
		...
		mVector<std::shared_ptr<Thread>> threadPool_;
